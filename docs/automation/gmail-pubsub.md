---
summary: "Gmail Pub/Sub push wired into JSEBot webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to JSEBot
  - Setting up Pub/Sub push for agent wake
title: "Gmail PubSub"
---

# Gmail Pub/Sub -> JSEBot

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> JSEBot webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- JSEBot hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the JSEBot helper to wire everything together (installs deps on macOS via brew):
...
Path note: when `tailscale.mode` is enabled, JSEBot automatically sets
...
- `--hook-url` points to JSEBot `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to JSEBot.
