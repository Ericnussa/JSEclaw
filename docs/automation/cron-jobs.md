...
To reduce top-of-hour load spikes across many gateways, JSEBot applies a
...
If `delivery` is omitted for isolated jobs, JSEBot defaults to `announce`.
...
When a job fails, JSEBot classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, JSEBot archives transcript files and purges old deleted archives on the same retention window.
...
- JSEBot applies exponential retry backoff for recurring jobs after consecutive errors:
