...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in JSEBot.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in JSEBot. See [Webhook Hooks](/automation/webhook) or use `openclaw webhooks` for Gmail helper commands.
...
- Extend JSEBot's behavior without modifying core code

## Getting Started

### Bundled Hooks

JSEBot ships with four bundled hooks that are automatically discovered:
...
3. **Bundled hooks**: `<openclaw>/dist/hooks/bundled/` (shipped with JSEBot)
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before JSEBot persists them.
