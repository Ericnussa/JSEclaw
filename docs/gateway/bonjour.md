...
JSEBot uses Bonjour (mDNS / DNS‑SD) as a **LAN‑only convenience** to discover
...
JSEBot supports any discovery domain; `openclaw.internal.` is just an example.
...
- Set `gateway.bind: "tailnet"` in `~/.jsebot/jsebot.json`.
...
- `gateway.bind` in `~/.jsebot/jsebot.json` controls the Gateway bind mode.
