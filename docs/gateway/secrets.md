...
JSEBot supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, JSEBot performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). JSEBot validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/JSEBot QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
...
When reload-time activation fails after a healthy state, JSEBot enters degraded secrets state.
...
JSEBot intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, JSEBot runtime no longer depends on plaintext `auth.json`.
