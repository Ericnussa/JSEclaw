...
read_when:
  - Running JSEBot behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of JSEBot
...
- You run JSEBot behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. JSEBot checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. JSEBot extracts the user identity from the configured header
...
- JSEBot can stay on loopback HTTP behind the proxy.
...
If JSEBot itself serves HTTPS directly (no TLS-terminating proxy), set:
...
3. Update JSEBot config with trusted-proxy auth
