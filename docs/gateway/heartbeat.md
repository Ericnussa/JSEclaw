...
- During heartbeat runs, JSEBot treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, JSEBot skips the heartbeat run entirely (no model call).
