...
- inject build metadata into Info.plist: `JSEBotBuildTimestamp` (UTC) and `JSEBotGitCommit` (short hash) so the About pane can show build, git, and debug/release channel.
...
- `JSEBotBuildTimestamp`: ISO8601 UTC at package time
- `JSEBotGitCommit`: short git hash (or `unknown` if unavailable)
...
TCC permissions are tied to the bundle identifier _and_ code signature. Unsigned debug builds with changing UUIDs were causing macOS to forget grants after each rebuild. Signing the binaries (ad‑hoc by default) and keeping a fixed bundle id/path (`dist/JSEBot.app`) preserves the grants between builds, matching the VibeTunnel approach.
