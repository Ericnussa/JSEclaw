---
summary: "JSEBot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up JSEBot on a Raspberry Pi
  - Running JSEBot on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# JSEBot on Raspberry Pi

## Goal

Run a persistent, always-on JSEBot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
If this Pi is mostly running JSEBot, add a service drop-in to reduce restart
...
If possible, keep JSEBot state/cache on SSD-backed storage to avoid SD-card
...
Most JSEBot features work on ARM64, but some external binaries may need ARM builds:
