...
This plan defines how JSEBot should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- JSEBot core owns ACP control plane concerns
...
JSEBot should not reimplement ACP transport internals in core.
JSEBot should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in JSEBot, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under JSEBot state dir
...
- ACP event stream is normalized to JSEBot reply chunks
...
For the first production backend (`extensions/acpx`), JSEBot and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses JSEBot `sessionKey`)
...
- JSEBot consumes ndjson events from `acpx --format json --json-strict`
