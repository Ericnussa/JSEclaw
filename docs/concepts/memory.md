---
title: "Memory"
summary: "How JSEBot memory works (workspace files + automatic memory flush)"
read_when:
...
JSEBot memory is **plain Markdown in the agent workspace**. The files are the
...
JSEBot exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, JSEBot triggers a **silent,
...
JSEBot can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, JSEBot auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. JSEBot
...
BM25 + vectors + reranking. Markdown stays the source of truth; JSEBot shells
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, JSEBot retries with `qmd query`. If QMD fails or the binary is
  missing, JSEBot automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- JSEBot does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - JSEBot sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index JSEBot
    uses), run a one-off query with the agent’s XDG dirs.

    JSEBot’s QMD state lives under your **state dir** (defaults to `~/.openclaw`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    JSEBot uses:

    ```bash
    # Pick the same state dir JSEBot uses
...
- When `scope` denies a search, JSEBot logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, JSEBot exports sanitized session
...
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, JSEBot automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, JSEBot combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, JSEBot falls back to vector-only search.
...
JSEBot can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
When the sqlite-vec extension is available, JSEBot stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, JSEBot logs the
