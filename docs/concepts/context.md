...
read_when:
  - You want to understand what “context” means in JSEBot
...
“Context” is **everything JSEBot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (JSEBot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How JSEBot builds the system prompt

The system prompt is **JSEBot-owned** and rebuilt each run. It includes:
...
By default, JSEBot injects a fixed set of workspace files (if present):
...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `20000` chars). JSEBot also enforces a total bootstrap injection cap across files with `agents.defaults.bootstrapTotalMaxChars` (default `150000` chars). `/context` shows **raw vs injected** sizes and whether truncation happened.
