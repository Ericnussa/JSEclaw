...
- Override in `~/.jsebot/jsebot.json`:
...
These are the standard files JSEBot expects inside the workspace:
...
If any bootstrap file is missing, JSEBot injects a "missing file" marker into
...
2. Set `agents.defaults.workspace` to that path in `~/.jsebot/jsebot.json`.
