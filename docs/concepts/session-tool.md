...
- When given a `sessionId`, JSEBot resolves it to the corresponding session key (missing ids error).
...
- After the primary run completes, JSEBot runs a **reply-back loop**:
  - Round 2+ alternates between requester and target agents.
  - Reply exactly `REPLY_SKIP` to stop the ping‑pong.
  - Max turns is `session.agentToAgent.maxPingPongTurns` (0–5, default 5).
- Once the loop ends, JSEBot runs the **agent‑to‑agent announce step** (target agent only):
...
- After completion, JSEBot runs a sub-agent **announce step** and posts the result to the requester chat channel.
...
- When a session is sandboxed and `sessionToolsVisibility="spawned"`, JSEBot clamps visibility to `tree` even if you set `tools.sessions.visibility="all"`.
