---
summary: "JSEBot Gateway CLI (`openclaw gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is JSEBot’s WebSocket server (channels, nodes, sessions, hooks).
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.jsebot/jsebot.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
