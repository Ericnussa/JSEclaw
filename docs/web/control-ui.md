...
- Config: view/edit `~/.jsebot/jsebot.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. JSEBot
...
JSEBot **blocks** Control UI connections without device identity.
