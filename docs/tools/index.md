---
summary: "Agent tool surface for JSEBot (browser, canvas, nodes, message, cron) replacing legacy `openclaw-*` skills"
read_when:
...
JSEBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
...
- If `tools.allow` only references unknown or unloaded plugin tool names, JSEBot logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openclaw`: all built-in JSEBot tools (excludes provider plugins)
...
JSEBot tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
Control the dedicated JSEBot-managed browser.
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, JSEBot uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, JSEBot clamps `tools.sessions.visibility` to `tree`.
