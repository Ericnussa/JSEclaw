---
title: Lobster
summary: "Typed workflow runtime for JSEBot with resumable approval gates."
description: Typed workflow runtime for JSEBot — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets JSEBot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: JSEBot runs one Lobster tool call and gets a structured result.
...
JSEBot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In JSEBot tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the JSEBot Gateway (see the [Lobster repo](https://github.com/openclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), JSEBot keeps core tools enabled. To restrict core
...
- **No secrets** — Lobster doesn't manage OAuth; it calls JSEBot tools that do.
