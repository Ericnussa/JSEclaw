...
ClawHub is the **public skill registry for JSEBot**. It is a free service: all skills are public, open, and visible to everyone for sharing and reuse. A skill is just a folder with a `SKILL.md` file (plus supporting text files). You can browse skills in the web app or use the CLI to search, install, update, and publish skills.

Site: [clawhub.ai](https://clawhub.ai)

## What ClawHub is

- A public registry for JSEBot skills.
...
4. Users browse, download, and install skills in JSEBot.
...
If you want to add new capabilities to your JSEBot agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
4. Start a new JSEBot session so it picks up the new skill.
...
## How it fits into JSEBot

By default, the CLI installs skills into `./skills` under your current working directory. If a JSEBot workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). JSEBot loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.openclaw/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches JSEBot how to perform a
...
Interested in becoming a moderator? Ask in the JSEBot Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to JSEBot workspace).
