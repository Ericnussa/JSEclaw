---
summary: "JSEBot plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends JSEBot with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core JSEBot yet (or you want to keep optional features out of your main
install).
...
JSEBot plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
4. Bundled extensions (shipped with JSEBot, **disabled by default**)
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, JSEBot logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. JSEBot blocks candidates when:
...
JSEBot can also merge **external channel catalogs** (for example, an MPM
...
If a plugin exports `id`, JSEBot uses it but warns when it doesn’t match the
configured id.
...
JSEBot augments `uiHints` at runtime based on discovered plugins:
...
If stored integrity metadata changes between updates, JSEBot warns and asks for confirmation (use global `--yes` to bypass prompts).
...
API-key setup inside JSEBot (no external scripts needed).
...
- `config`: The current JSEBot config
