...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let JSEBot run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask JSEBot in plain language to "run this in Codex" or "start Claude Code in a thread", JSEBot should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want JSEBot-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | JSEBot native sub-agent runtime  |
...
- JSEBot binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, JSEBot returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, JSEBot uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, JSEBot may default to persistent behavior per runtime path
...
If no target resolves, JSEBot returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, JSEBot returns a clear unsupported-control error.
...
When JSEBot uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal JSEBot `agentId` path).
...
- Relative paths resolve from JSEBot workspace directory.
...
- JSEBot startup remains non-blocking while the backend health check runs.
...
> **Important:** JSEBot currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
