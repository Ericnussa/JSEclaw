...
- For manual spawns, delivery is resilient:
  - JSEBot tries direct `agent` delivery first with a stable idempotency key.
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, JSEBot uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
2. JSEBot creates or binds a thread to that session target in the active channel.
