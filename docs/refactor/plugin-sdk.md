...
Accessed via `JSEBotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: JSEBotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: JSEBotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: JSEBotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: JSEBotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: JSEBotConfig): string;
  };
};
```
...
- Add `api.runtime` to `JSEBotPluginApi` with the surface above.
