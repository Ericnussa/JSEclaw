...
6. **Enable the app status**:
   ...
   - Click **Save** again.
7. Configure JSEBot with the service account path + webhook audience:
...
Google Chat webhooks require a public HTTPS endpoint. For security, **only expose the `/googlechat` path** to the internet. Keep the JSEBot dashboard and other sensitive endpoints on your private network.
...
With this config, any request to `your-domain.com/` will be ignored or returned as 404, while `your-domain.com/googlechat` is safely routed to JSEBot.
...
1. Google Chat sends webhook POSTs to the gateway. Each request includes an `Authorization: Bearer <token>` header.
   - JSEBot verifies bearer auth before reading/parsing full webhook bodies when the header is present.
...
2. JSEBot verifies the token against the configured `audienceType` + `audience`:
