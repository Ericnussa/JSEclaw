...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects JSEBot to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
If you just installed JSEBot, run the wizard:
...
2. Configuring app credentials in JSEBot
