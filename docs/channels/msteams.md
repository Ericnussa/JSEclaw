...
JSEBot will offer the local install path automatically.
...
3. Configure JSEBot with those credentials.
...
- Talk to JSEBot via Teams DMs, group chats, or channels.
...
- On startup, JSEBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.jsebot/jsebot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring JSEBot, you need to create an Azure Bot resource.
...
```json
{
  ...
  "description": { "short": "JSEBot in Teams", "full": "JSEBot in Teams" },
...
JSEBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, JSEBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/JSEBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

JSEBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).
