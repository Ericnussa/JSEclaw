...
```
    ...
    Telegram does **not** use `jsebot channels login telegram`; configure token in config/env, then start gateway.
...
- DM messages can carry `message_thread_id`; JSEBot routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    JSEBot can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, JSEBot keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), JSEBot falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, JSEBot skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, JSEBot retries as plain text.
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, JSEBot enqueues system events like:
...
<AccordionGroup>
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, JSEBot now retries these as recoverable network errors.
