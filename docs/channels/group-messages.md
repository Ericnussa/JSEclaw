...
Add a `groupChat` block to `~/.jsebot/jsebot.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
...
1. Add your WhatsApp account (the one running JSEBot) to the group.
