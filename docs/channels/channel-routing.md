...
JSEBot routes replies **back to the channel where a message came from**. The
...
JSEBot infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, JSEBot still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when JSEBot would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
