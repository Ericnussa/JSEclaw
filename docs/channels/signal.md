...
- JSEBot installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "JSEBot"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure JSEBot and restart the gateway.
...
2. Link a bot account:
   - `signal-cli link -n "JSEBot"` then scan the QR in Signal.
...
4. Configure JSEBot, restart gateway, verify channel:
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point JSEBot at it:
...
This skips auto-spawn and the startup wait inside JSEBot. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- **Typing indicators**: JSEBot sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, JSEBot forwards read receipts for allowed DMs.
