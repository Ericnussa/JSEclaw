...
```
  ...
  <Tab title="Remote Mac over SSH">
    JSEBot only requires a stdio-compatible `cliPath`, so you can point `cliPath` at a wrapper script that SSHes to a remote Mac and runs `imsg`.
...
```
    ...
    If `remoteHost` is not set, JSEBot attempts to auto-detect it by parsing the SSH wrapper script.
    `remoteHost` must be `host` or `user@host` (no spaces or SSH options).
    JSEBot uses strict host-key checking for SCP, so the relay host key must already exist in `~/.ssh/known_hosts`.
...
- Full Disk Access is required for the process context running JSEBot/`imsg` (Messages DB access).
...
<Tabs>
  ...
  <Tab title="Sessions and deterministic replies">
    ...
    If that `chat_id` is explicitly configured under `channels.imessage.groups`, JSEBot treats it as group traffic (group gating + group session isolation).
...
<AccordionGroup>
  <Accordion title="Dedicated bot macOS user (separate iMessage identity)">
    ...
    4. Create SSH wrapper so JSEBot can run `imsg` in that user context.
...
```
    ...
    Confirm Full Disk Access + Automation are granted for the process context that runs JSEBot/`imsg`.
