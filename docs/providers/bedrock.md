---
summary: "Use Amazon Bedrock (Converse API) models with JSEBot"
read_when:
  - You want to use Amazon Bedrock models with JSEBot
...
JSEBot can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, JSEBot can automatically discover Bedrock
...
When running JSEBot on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, JSEBot's credential detection currently only checks for environment
...
- JSEBot surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
