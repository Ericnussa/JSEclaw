---
summary: "Use Synthetic's Anthropic-compatible API in JSEBot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. JSEBot registers it as the
...
Note: JSEBot's Anthropic client appends `/v1` to the base URL, so use
