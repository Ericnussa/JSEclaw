---
summary: "Use Anthropic Claude via API keys or setup-token in JSEBot"
read_when:
  - You want to use Anthropic models in JSEBot
...
In JSEBot you can authenticate with an API key or a **setup-token**.
...
- Anthropic Claude 4.6 models default to `adaptive` thinking in JSEBot when no explicit thinking level is set.
...
JSEBot supports Anthropic's prompt caching feature. This is **API-only**; subscription auth does not honor cache settings.
...
When using Anthropic API Key authentication, JSEBot automatically applies `cacheRetention: "short"` (5-minute cache) for all Anthropic models. You can override this by explicitly setting `cacheRetention` in your config.
...
JSEBot includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
requests; keep it if you override provider headers (see [/gateway/configuration](/gateway/configuration)).

## 1M context window (Anthropic beta)

Anthropic's 1M context window is beta-gated. In JSEBot, enable it per model
...
JSEBot maps this to `anthropic-beta: context-1m-2025-08-07` on Anthropic
requests.
...
OAuth/subscription tokens (`sk-ant-oat-*`). JSEBot automatically skips the
...
Paste the token into JSEBot (wizard: **Anthropic token (paste setup-token)**), or run it on the gateway host:
