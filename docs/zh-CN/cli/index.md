...
read_when:
  - 添加或修改 CLI 命令或选项
  - 为新命令界面编写文档
summary: JSEBot `openclaw` 命令、子命令和选项的 CLI 参考
title: CLI 参考
x-i18n:
...
- `jsebot security audit --deep` — 尽力进行实时 Gateway 网关探测。
...
当 OAuth/API 凭证可用时，JSEBot 可以显示提供商用量/配额。
...
- `gateway status` 在检测到旧版或额外的 Gateway 网关服务时也会显示（`--deep` 添加系统级扫描）。配置文件命名的 JSEBot 服务被视为一等公民，不会被标记为"额外"。
