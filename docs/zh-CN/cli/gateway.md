...
summary: JSEBot Gateway 网关 CLI（`openclaw gateway`）— 运行、查询和发现 Gateway 网关
title: gateway
x-i18n:
...
Gateway 网关是 JSEBot 的 WebSocket 服务器（渠道、节点、会话、hooks）。
...
- 默认情况下，除非在 `~/.jsebot/jsebot.json` 中设置了 `gateway.mode=local`，否则 Gateway 网关将拒绝启动。使用 `--allow-unconfigured` 进行临时/开发运行。
