...
运行与 JSEBot Gateway 网关通信的 ACP（Agent Client Protocol）桥接器。
...
当 IDE（或其他客户端）使用 Agent Client Protocol 并且你希望它驱动 JSEBot Gateway 网关会话时，请使用 ACP。
...
在 Zed 中，打开 Agent 面板并选择"JSEBot ACP"来开始一个会话。
