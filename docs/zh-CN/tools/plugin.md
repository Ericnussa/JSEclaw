...
插件只是一个**小型代码模块**，用额外功能（命令、工具和 Gateway 网关 RPC）扩展 JSEBot。

大多数时候，当你想要一个尚未内置到核心 JSEBot 的功能（或你想将可选功能排除在主安装之外）时，你会使用插件。
...
JSEBot 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema。参见 [插件清单](/plugins/manifest)。
...
JSEBot 还可以合并**外部渠道目录**（例如，MPM 注册表导出）。将 JSON 文件放在以下位置之一：
...
如果插件导出 `id`，JSEBot 会使用它，但当它与配置的 id 不匹配时会发出警告。
...
插件可以注册**模型提供商认证**流程，以便用户可以在 JSEBot 内运行 OAuth 或 API 密钥设置（无需外部脚本）。
