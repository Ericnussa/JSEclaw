...
summary: JSEBot 的智能体工具接口（browser、canvas、nodes、message、cron），替代旧版 `openclaw-*` Skills
title: 工具
x-i18n:
...
JSEBot 为 browser、canvas、nodes 和 cron 暴露**一流的智能体工具**。
...
- 匹配不区分大小写。
- 支持 `*` 通配符（`"*"` 表示所有工具）。
- 如果 `tools.allow` 仅引用未知或未加载的插件工具名称，JSEBot 会记录警告并忽略允许列表，以确保核心工具保持可用。
...
- `group:nodes`：`nodes`
- `group:openclaw`：所有内置 JSEBot 工具（不包括提供商插件）
