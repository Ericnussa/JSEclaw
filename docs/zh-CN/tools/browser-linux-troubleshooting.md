...
summary: 修复 Linux 上 JSEBot 浏览器控制的 Chrome/Brave/Edge/Chromium CDP 启动问题
title: 浏览器故障排除
x-i18n:
...
JSEBot 的浏览器控制服务器无法启动 Chrome/Brave/Edge/Chromium，出现以下错误：
...
在 Ubuntu（和许多 Linux 发行版）上，默认的 Chromium 安装是 **snap 包**。Snap 的 AppArmor 限制会干扰 JSEBot 启动和监控浏览器进程的方式。
...
然后更新你的 JSEBot 配置（`~/.jsebot/jsebot.json`）：
...
如果你必须使用 snap Chromium，配置 JSEBot 附加到手动启动的浏览器：
