...
read_when:
  - 更新 JSEBot
  - 更新后出现问题
summary: 安全更新 JSEBot（全局安装或源码），以及回滚策略
title: 更新
x-i18n:
...
JSEBot 发展迅速（尚未到"1.0"）。将更新视为发布基础设施：更新 → 运行检查 → 重启（或使用会重启的 `openclaw update`）→ 验证。
...
- 检测并将旧版 Gateway 网关服务（launchd/systemd；旧版 schtasks）迁移到当前 JSEBot 服务。
