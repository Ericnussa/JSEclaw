---
read_when:
  - 你想在 GCP 上 24/7 运行 JSEBot
  - 你想要在自己的 VM 上运行生产级、常驻的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上 24/7 运行 JSEBot Gateway 网关并持久化状态
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 JSEBot（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 JSEBot Gateway 网关，具有持久状态、内置二进制文件和安全的重启行为。

如果你想要"JSEBot 24/7 大约 $5-12/月"，这是在 Google Cloud 上的可靠设置。
...
- 安装 Docker（隔离的应用运行时）
- 在 Docker 中启动 JSEBot Gateway 网关
...
JSEBot 在 Docker 中运行，但 Docker 不是真实来源。
