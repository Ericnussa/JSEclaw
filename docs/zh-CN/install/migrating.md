---
read_when:
  - 你正在将 JSEBot 迁移到新的笔记本电脑/服务器
  - 你想保留会话、认证和渠道登录（WhatsApp 等）
summary: 将 JSEBot 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
本指南将 JSEBot Gateway 网关从一台机器迁移到另一台，**无需重新进行新手引导**。
...
- [JSEBot 在哪里存储数据？](/help/faq#where-does-openclaw-store-its-data)
