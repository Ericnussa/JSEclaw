...
read_when:
  - 启动新的 JSEBot 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 JSEBot 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — JSEBot 个人助手（默认）

## 首次运行（推荐）

JSEBot 为智能体使用专用的工作区目录。默认：`~/.openclaw/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
- 对于浏览器驱动的验证，使用带有 JSEBot 管理的 Chrome 配置文件的 `openclaw browser`（tabs/status/screenshot）。
