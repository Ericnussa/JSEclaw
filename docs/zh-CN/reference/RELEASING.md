...
- **npm pack/publish 挂起或产生巨大 tarball**：`dist/JSEBot.app` 中的 macOS 应用包（和发布 zip）被扫入包中。通过 `package.json` 的 `files` 白名单发布内容来修复（包含 dist 子目录、docs、skills；排除应用包）。用 `npm pack --dry-run` 确认 `dist/JSEBot.app` 未列出。
...
- [ ] 附加产物：`npm pack` tarball（可选）、`JSEBot-X.Y.Z.zip` 和 `JSEBot-X.Y.Z.dSYM.zip`（如果生成）。
