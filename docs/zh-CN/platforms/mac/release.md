---
read_when:
  - 制作或验证 JSEBot macOS 发布版本
  - 更新 Sparkle appcast 或订阅源资源
summary: JSEBot macOS 发布清单（Sparkle 订阅源、打包、签名）
title: macOS 发布
x-i18n:
...
ditto -c -k --sequesterRsrc --keepParent dist/JSEBot.app dist/JSEBot-2026.1.27-beta.1.zip

# 可选：同时构建适合用户使用的样式化 DMG（拖拽到 /Applications）
scripts/create-dmg.sh dist/JSEBot.app dist/JSEBot-2026.1.27-beta.1.dmg
...
# 可选：随发布一起提供 dSYM
ditto -c -k --keepParent apps/macos/.build/release/JSEBot.app.dSYM dist/JSEBot-2026.1.27-beta.1.dSYM.zip
```
...
```bash
SPARKLE_PRIVATE_KEY_FILE=/path/to/ed25519-private-key scripts/make_appcast.sh dist/JSEBot-2026.1.27-beta.1.zip https://raw.githubusercontent.com/openclaw/openclaw/main/appcast.xml
```
...
- 将 `JSEBot-2026.1.27-beta.1.zip`（和 `JSEBot-2026.1.27-beta.1.dSYM.zip`）上传到标签 `v2026.1.27-beta.1` 对应的 GitHub 发布。
