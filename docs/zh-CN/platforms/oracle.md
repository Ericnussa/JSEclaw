---
read_when:
  - 在 Oracle Cloud 上设置 JSEBot
  - 寻找 JSEBot 的低成本 VPS 托管
  - 想要在小型服务器上 24/7 运行 JSEBot
summary: 在 Oracle Cloud 上运行 JSEBot（Always Free ARM）
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud（OCI）上运行 JSEBot

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层上运行持久化的 JSEBot Gateway 网关。

Oracle 的免费层非常适合 JSEBot（特别是如果你已经有 OCI 账户），但有一些权衡：
