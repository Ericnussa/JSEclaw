---
read_when:
  - 你想在 JSEBot 中使用 Anthropic 模型
  - 你想使用 setup-token 而不是 API 密钥
summary: 在 JSEBot 中通过 API 密钥或 setup-token 使用 Anthropic Claude
title: Anthropic
x-i18n:
...
在 JSEBot 中，你可以使用 API 密钥或 **setup-token** 进行认证。
...
JSEBot 支持 Anthropic 的提示缓存功能。这是**仅限 API**；订阅认证不支持缓存设置。
...
使用 Anthropic API 密钥认证时，JSEBot 会自动为所有 Anthropic 模型应用 `cacheRetention: "short"`（5 分钟缓存）。你可以通过在配置中显式设置 `cacheRetention` 来覆盖此设置。
...
JSEBot 在 Anthropic API 请求中包含 `extended-cache-ttl-2025-04-11` beta 标志；
...
将令牌粘贴到 JSEBot（向导：**Anthropic token (paste setup-token)**），或在 Gateway 网关主机上运行：
