...
如果任何引导文件缺失，JSEBot 会在会话中注入"缺失文件"标记并继续。大型引导文件在注入时会被截断；使用 `agents.defaults.bootstrapMaxChars` 调整限制（默认：20000）。
...
2. 在 `~/.jsebot/jsebot.json` 中将 `agents.defaults.workspace` 设置为该路径。
