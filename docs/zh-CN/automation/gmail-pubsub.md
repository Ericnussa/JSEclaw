---
read_when:
  - 将 Gmail 收件箱触发器接入 JSEBot
  - 为智能体唤醒设置 Pub/Sub 推送
summary: 通过 gogcli 将 Gmail Pub/Sub 推送接入 JSEBot webhooks
title: Gmail PubSub
x-i18n:
...
目标：Gmail watch -> Pub/Sub 推送 -> `gog gmail watch serve` -> JSEBot webhook。
...
- 已启用 JSEBot hooks（参见 [Webhooks](/automation/webhook)）。
...
使用 JSEBot 助手将所有内容接入在一起（在 macOS 上通过 brew 安装依赖）：
...
路径说明：当启用 `tailscale.mode` 时，JSEBot 会自动将 `hooks.gmail.serve.path` 设置为 `/`，并将公共路径保持在 `hooks.gmail.tailscale.path`（默认 `/gmail-pubsub`），因为 Tailscale 在代理之前会剥离设置的路径前缀。
...
- `--hook-url` 指向 JSEBot `/hooks/gmail`（已映射；隔离运行 + 摘要发送到主线程）。
- `--include-body` 和 `--max-bytes` 控制发送到 JSEBot 的正文片段。
