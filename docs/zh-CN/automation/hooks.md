...
Hooks 提供了一个可扩展的事件驱动系统，用于响应智能体命令和事件自动执行操作。Hooks 从目录中自动发现，可以通过 CLI 命令管理，类似于 JSEBot 中 Skills 的工作方式。
...
- **Webhooks**：外部 HTTP webhooks，让其他系统触发 JSEBot 中的工作。参见 [Webhook Hooks](/automation/webhook) 或使用 `openclaw webhooks` 获取 Gmail 助手命令。
...
- 记录所有命令以供审计
- 在智能体生命周期事件上触发自定义自动化
- 在不修改核心代码的情况下扩展 JSEBot 的行为

## 入门

### 捆绑的 Hooks

JSEBot 附带三个自动发现的捆绑 hooks：
...
3. **捆绑 hooks**：`<openclaw>/dist/hooks/bundled/`（随 JSEBot 附带）
...
这些 hooks 不是事件流监听器；它们让插件在 JSEBot 持久化工具结果之前同步调整它们。
