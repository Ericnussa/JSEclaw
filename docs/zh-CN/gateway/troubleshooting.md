...
JSEBot 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。
...
jsebot channels login --verbose       # 重新扫描二维码
```
...
当配置包含未知键、格式错误的值或无效类型时，JSEBot 现在拒绝启动。
这是为了安全而故意设计的。
...
端点接受 JSON Schema 的严格子集。JSEBot 在当前 `main` 中清理/规范化工具
...
| Gateway 网关服务日志（监管程序） | macOS：`$OPENCLAW_STATE_DIR/logs/gateway.log` + `gateway.err.log`（默认：`~/.openclaw/logs/...`；配置文件使用 `~/.openclaw-<profile>/logs/...`）<br />Linux：`journalctl --user -u openclaw-gateway[-<profile>].service -n 200 --no-pager`<br />Windows：`schtasks /Query /TN "JSEBot Gateway (<profile>)" /V /FO LIST` |
...
jsebot channels login         # 重新配对 WhatsApp
