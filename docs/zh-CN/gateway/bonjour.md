...
JSEBot 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
JSEBot 支持任何发现域名；`openclaw.internal.` 只是一个示例。
...
- 在 `~/.jsebot/jsebot.json` 中设置 `gateway.bind: "tailnet"`。
...
- `~/.jsebot/jsebot.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
