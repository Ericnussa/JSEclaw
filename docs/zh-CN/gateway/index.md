...
- 配置热重载监视 `~/.jsebot/jsebot.json`（或 `OPENCLAW_CONFIG_PATH`）。
...
- Windows：`JSEBot Gateway (<profile>)`
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 JSEBot 配置文件服务。
...
- JSEBot.app 可以捆绑基于 Node 的 Gateway 网关中继并安装标记为
...
JSEBot 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
...
```
[Unit]
Description=JSEBot Gateway (profile: <profile>, v<version>)
