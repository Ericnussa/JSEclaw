...
read_when:
  - 添加或修改配置字段时
summary: ~/.jsebot/jsebot.json 的所有配置选项及示例
title: 配置
x-i18n:
...
JSEBot 从 `~/.jsebot/jsebot.json` 读取可选的 **JSON5** 配置（支持注释和尾逗号）。

如果文件不存在，JSEBot 使用安全的默认值（内置 Pi 智能体 + 按发送者分会话 + 工作区 `~/.openclaw/workspace`）。通常只在以下情况需要配置：
...
请使用 `config.patch` 或 `openclaw config set`。请备份 `~/.jsebot/jsebot.json`。
...
JSEBot 从父进程（shell、launchd/systemd、CI 等）读取环境变量。
...
可选便利功能：如果启用且预期键均未设置，JSEBot 会运行你的登录 shell 并仅导入缺失的预期键（不会覆盖）。
...
JSEBot 在以下位置存储**每个智能体的**认证配置文件（OAuth + API 密钥）：
...
首次使用时，JSEBot 会将 `oauth.json` 条目导入到 `auth-profiles.json` 中。
...
JSEBot 仅在存在 `channels.telegram` 配置段时启动 Telegram。机器人 token 从 `channels.telegram.botToken`（或 `channels.telegram.tokenFile`）解析，`TELEGRAM_BOT_TOKEN` 作为默认账号的回退。
...
JSEBot 仅在存在 `channels.discord` 配置段时启动 Discord。token 从 `channels.discord.token` 解析，`DISCORD_BOT_TOKEN` 作为默认账号的回退（除非 `channels.discord.enabled` 为 `false`）。在为 cron/CLI 命令指定投递目标时，使用 `user:<id>`（私聊）或 `channel:<id>`（服务器频道）；裸数字 ID 有歧义会被拒绝。
...
JSEBot 在提供商启用且两个 token 都已设置时启动 Slack（通过配置或 `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`）。在为 cron/CLI 命令指定投递目标时使用 `user:<id>`（私聊）或 `channel:<id>`。
...
JSEBot 在账号已配置（机器人 token + 基础 URL）且已启用时启动 Mattermost。token + 基础 URL 从 `channels.mattermost.botToken` + `channels.mattermost.baseUrl` 或默认账号的 `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` 解析（除非 `channels.mattermost.enabled` 为 `false`）。
...
JSEBot 会生成 `imsg rpc`（通过 stdio 的 JSON-RPC）。无需守护进程或端口。
...
在系统提示的 Runtime 行中显示的可选仓库根目录。如果未设置，JSEBot 会从工作区（和当前工作目录）向上查找 `.git` 目录进行检测。路径必须存在才能使用。
...
当文件超过此限制时，JSEBot 会记录警告并注入带标记的头尾截断内容。
...
设置用户时区用于**系统提示上下文**（不用于消息信封中的时间戳）。如果未设置，JSEBot 在运行时使用主机时区。
...
WhatsApp 入站前缀通过 `channels.whatsapp.messagePrefix` 配置（已弃用：`messages.messagePrefix`）。默认保持**不变**：当 `channels.whatsapp.allowFrom` 为空时为 `"[openclaw]"`，否则为 `""`（无前缀）。使用 `"[openclaw]"` 时，如果路由的智能体设置了 `identity.name`，JSEBot 会改用 `[{identity.name}]`。
...
为出站回复启用文字转语音。开启后，JSEBot 使用 ElevenLabs 或 OpenAI 生成音频并附加到回复中。Telegram 使用 Opus 语音消息；其他渠道发送 MP3 音频。
...
JSEBot 还内置了一些别名快捷方式。默认值仅在模型已存在于 `agents.defaults.models` 中时才应用：
...
如果省略提供商，JSEBot 目前假定 `anthropic` 作为临时弃用回退。
...
- `group:nodes`：`nodes`
- `group:openclaw`：所有内置 JSEBot 工具（不包含提供商插件）
...
JSEBot 使用 **pi-coding-agent** 模型目录。你可以通过编写
`~/.openclaw/agents/<agentId>/agent/models.json` 或在 JSEBot 配置中的 `models.providers` 下定义相同的 schema 来添加自定义提供商（LiteLLM、本地 OpenAI 兼容服务器、Anthropic 代理等）。
按提供商的概述 + 示例：[/concepts/model-providers](/concepts/model-providers)。

当存在 `models.providers` 时，JSEBot 在启动时将 `models.json` 写入/合并到
...
OpenCode Zen 是一个具有每模型端点的多模型网关。JSEBot 使用
...
- `resetByType`：`dm`、`group` 和 `thread` 的每会话覆盖。
  - 如果你只设置了旧版 `session.idleMinutes` 而没有任何 `reset`/`resetByType`，JSEBot 保持仅空闲模式以向后兼容。
...
### `browser`（JSEBot 管理的浏览器）

JSEBot 可以为 JSEBot 启动一个**专用、隔离的** Chrome/Brave/Edge/Chromium 实例并暴露一个小型 local loopback 控制服务。
...
- 配置文件颜色：`#FF4500`（龙虾橙）
- 注意：控制服务器由运行中的 Gateway 网关（JSEBot.app 菜单栏或 `openclaw gateway`）启动。
...
- 当连接来自这些 IP 之一时，JSEBot 使用 `x-forwarded-for`（或 `x-real-ip`）来确定客户端 IP，用于本地配对检查和 HTTP 认证/本地检查。
...
- `gateway.auth.allowTailscale` 允许 Tailscale Serve 身份头部
  ...
  `x-forwarded-proto` 和 `x-forwarded-host` 时满足认证。JSEBot 在接受之前
...
- JSEBot.app 监视 `~/.jsebot/jsebot.json`，当 `gateway.mode` 或 `gateway.remote.url` 变更时实时切换模式。
...
Gateway 网关监视 `~/.jsebot/jsebot.json`（或 `OPENCLAW_CONFIG_PATH`）并自动应用更改。
...
- `~/.jsebot/jsebot.json`（或 `OPENCLAW_CONFIG_PATH`）
...
注意：当 `tailscale.mode` 开启时，JSEBot 将 `serve.path` 默认为 `/`，以便
...
默认端口：`18793`（选择此端口以避免 JSEBot 浏览器 CDP 端口 `18792`）
