...
2. 在 `~/.jsebot/jsebot.json` 中配置 WhatsApp。
3. 运行 `jsebot channels login` 扫描二维码（关联设备）。
4. 启动 Gateway 网关。
...
WhatsApp 需要真实手机号码进行验证。VoIP 和虚拟号码通常会被封锁。在 WhatsApp 上运行 JSEBot 有两种支持的方式：

### 专用号码（推荐）

为 JSEBot 使用**单独的手机号码**。最佳用户体验，清晰的路由，无自聊天怪异问题。理想设置：**备用/旧 Android 手机 + eSIM**。保持 Wi-Fi 和电源连接，通过二维码关联。

**WhatsApp Business：** 你可以在同一设备上使用不同号码的 WhatsApp Business。非常适合将个人 WhatsApp 分开——安装 WhatsApp Business 并在那里注册 JSEBot 号码。
...
快速备选方案：在**你自己的号码**上运行 JSEBot。给自己发消息（WhatsApp"给自己发消息"）进行测试，这样就不会打扰联系人。在设置和实验期间需要在主手机上阅读验证码。**必须启用自聊天模式。**
...
- 早期 JSEBot 版本支持 Twilio 的 WhatsApp Business 集成。
...
- 登录命令：`jsebot channels login`（通过关联设备扫描二维码）。
- 多账户登录：`jsebot channels login --account <id>`（`<id>` = `accountId`）。
...
如果你在**个人 WhatsApp 号码**上运行 JSEBot，请启用 `channels.whatsapp.selfChatMode`（见上面的示例）。
...
**当我关联 WhatsApp 时，JSEBot 会给随机联系人发消息吗？**
不会。默认私信策略是**配对**，因此未知发送者只会收到配对码，他们的消息**不会被处理**。JSEBot 只会回复它收到的聊天，或你明确触发的发送（智能体/CLI）。
...
**多个人可以在一个 WhatsApp 号码上使用不同的 JSEBot 实例吗？**
...
- 最佳效果：OGG/Opus。JSEBot 将 `audio/ogg` 重写为 `audio/ogg; codecs=opus`。
...
- 修复：在 Gateway 网关主机上运行 `jsebot channels login` 并扫描二维码（WhatsApp → 设置 → 关联设备）。
