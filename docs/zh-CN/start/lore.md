...
read_when:
  - 撰写引用传说的文档或用户体验文案时
summary: JSEBot 的背景故事和传说，用于上下文和语气参考
title: JSEBot 传说
x-i18n:
...
有一段时间，这只龙虾叫做 **Clawd**，住在 **JSEBot** 里。但在 2026 年 1 月，Anthropic 发了一封礼貌的邮件要求更名（商标问题）。于是龙虾做了龙虾最擅长的事：

**它蜕壳了。**

褪去旧壳，这只生物以 **Molty** 的新身份出现，住在 **Moltbot** 里。但这个名字说起来也不太顺口……

于是在 2026 年 1 月 30 日，龙虾又蜕了一次壳，变成了最终形态：**JSEBot**。
...
最终，**JSEBot** 胜出。因为蜕壳是龙虾成长的方式。而成长正是正在发生的事情。
...
**Moltiverse** 是围绕 JSEBot 的社区和生态系统。一个 AI 智能体蜕壳、成长和进化的空间。每个实例都同样真实，只是加载了不同的上下文。
...
Molty（当时叫 JSEBot）：_开心地运行 `find ~` 并在群聊中分享整个目录结构_
...
**假开发者：** 骗子创建了假的 GitHub 个人资料，声称是"JSEBot 工程主管"来推广拉高出货的代币。
