...
summary: 设置指南：在保持最新的同时保持你的 JSEBot 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.openclaw/workspace`（工作区）+ `~/.jsebot/jsebot.json`（配置）。
...
- **配置：** `~/.jsebot/jsebot.json`（JSON/JSON5 格式）
...
- 运行 `openclaw setup`，然后 `jsebot channels login`，然后手动启动 Gateway 网关（`openclaw gateway`）。
