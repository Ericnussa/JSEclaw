...
read_when:
  - 向新用户介绍 JSEBot
summary: JSEBot 是一个多渠道 AI 智能体 Gateway 网关，可在任何操作系统上运行。
title: JSEBot
x-i18n:
...
JSEBot 通过单个 Gateway 网关进程将聊天应用连接到 Pi 等编程智能体。它为 JSEBot 助手提供支持，并支持本地或远程部署。
...
配置文件位于 `~/.jsebot/jsebot.json`。

- 如果你**不做任何修改**，JSEBot 将使用内置的 Pi 二进制文件以 RPC 模式运行，并按发送者创建独立会话。
