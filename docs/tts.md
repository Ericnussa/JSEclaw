...
JSEBot can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere JSEBot can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, JSEBot defaults
...
- If `provider` is **unset**, JSEBot prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, JSEBot retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so JSEBot registers
