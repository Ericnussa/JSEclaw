...
This document explains how JSEBot manages sessions end-to-end:
...
JSEBot is designed around a single **Gateway process** that owns session state.
...
JSEBot persists sessions in two layers:
...
JSEBot resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, JSEBot reports potential evictions but does not mutate the store/files.
...
JSEBot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (JSEBot consumes the events, but Pi decides when to compact).
...
JSEBot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, JSEBot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, JSEBot leaves it alone.
...
JSEBot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- JSEBot strips/suppresses this in the delivery layer.

As of `2026.1.10`, JSEBot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
JSEBot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but JSEBot’s
