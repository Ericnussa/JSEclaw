...
JSEBot integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- JSEBot owns lifecycle when `channels.signal.autoStart=true`.
...
- JSEBot spawns `imsg rpc` as a child process (legacy iMessage integration).
