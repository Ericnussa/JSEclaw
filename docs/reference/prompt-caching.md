...
- With Anthropic API-key auth profiles, JSEBot seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, JSEBot injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
JSEBot exposes dedicated cache-trace diagnostics for embedded agent runs.
