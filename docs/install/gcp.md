---
summary: "Run JSEBot Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want JSEBot running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# JSEBot on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent JSEBot Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "JSEBot 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the JSEBot Gateway in Docker
...
```bash
gcloud projects create my-openclaw-project --name="JSEBot Gateway"
...
JSEBot runs in Docker, but Docker is not the source of truth.
...
1. Create a service account:
   ...
   gcloud iam service-accounts create openclaw-deploy \
     --display-name="JSEBot Deployment"
   ```
